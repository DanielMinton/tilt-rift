...
const fragmentShader = `
  ...
  void main() {
    ...
    color = mix(color, uColor3, pow(combined, 2.0) * 0.5);
...
export function BackgroundRift() {
  ...
  const uniforms = useMemo(
    () => ({
      uTime: { value: 0 },
      uColor1: { value: new Color('#07060D') },
      uColor2: { value: new Color('#0B1020') },
      uColor3: { value: new Color('#1FF2FF').multiplyScalar(0.1) },
  ...
  useFrame((state) => {
    if (materialRef.current) {
      ...
      // Adjust intensity based on overclock
      const targetIntensity = overclockEnabled ? 0.5 : 0.3;
